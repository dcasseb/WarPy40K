...
from warpy40k import WarPy40KError, evaluate
...
class TestInterpreter:
    ...
    def test_builtin_conversions(self):
        """Test explicit conversion from input-like strings and numbers."""
        assert evaluate('int(" 42 ")') == 42
        assert evaluate('float("2.5")') == 2.5
        assert evaluate("str(42)") == "42"

    def test_invalid_numeric_conversion_is_reported(self):
        """Test invalid text does not silently become a number."""
        with pytest.raises(WarPy40KError, match="invalid literal"):
            evaluate('int("forty-two")')
