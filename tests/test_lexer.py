...
from warpy40k.tokens import Token, TokenType
...
class TestLexer:
    ...
    def test_punctuation(self):
        ...
        expected_types = [
            ...
            TokenType.RBRACE, TokenType.SEMICOLON, TokenType.COLON,
            TokenType.COMMA, TokenType.ASSIGN, TokenType.EOF
        ]
        for i, expected in enumerate(expected_types):
            assert tokens[i].type == expected
    ...
    def test_whitespace_ignored(self):
        ...
        lexer = Lexer("  42  ")
        tokens = lexer.tokenize()
        # Should only have INTEGER and EOF, no WHITESPACE
        assert len(tokens) == 2
        assert tokens[0].type == TokenType.INTEGER
        assert tokens[1].type == TokenType.EOF
    ...
    def test_comments(self):
        ...
        assert "This is a comment" in tokens[1].value
